...
export class Host {
    ...
    constructor(private window: Window, private readonly platform: Platform, private commands: BehaviorSubject<Array<Command>>, protected modulesMap: Map<string, {module: Module, window: Window, eventEmitter: Subject<PlatformEvent>, platform: Platform}> = new Map()) {
    ...
    public registerCommand(command_name: string, callback: (...args: any[]) => void, meta: Record<string, unknown> = {}): {remove: ()=>void} {
    ...
    public callCommand(command_name: string, ...args: any) {
        ...
        if(commands.length === 0) console.log(`Command [${command_name}] not registered!`)
    ...
    public exec(filepath: string, ...args: string[]) {
        ...
        let script: string = ''
        if(stat.isDirectory()) {
            script = `service('001-core.layout', 'open-window') (command('ui.file-explorer'), '${filepath}' ${args.length?',':''} ${args.map(x => `"${x}"`).join(', ')})`;
            // console.log(script)
    ...
    public getModulesDetails(){
        ...
        this.modulesMap.forEach(item => mp[item.platform.name] = item.platform.getServiceList())
        return mp
    }
}
