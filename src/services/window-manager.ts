...
export class WindowManager {
    private readonly windows: Record<string, Array<{container: HTMLDivElement}>> = {}
    ...
    public createWindow(command_name: string, ...args: unknown[]) {
        ...
        const [head, closeButton, fullScreenButton, setTitle, appendActionButton] = createWindoeHeader(command)
        ...
        iframe.onload = () => {
            ...
            command.exec(iframeBody, {
                ...
                setTitle,
                appendActionButton,
                setWindowView: (show: boolean) => show ? container.classList.remove('hidden') : container.classList.add('hidden'),
                ...
                setBoundingClientRect: (rect: Record<string, number>) => {
                    Object.keys(rect).forEach((attr: any) => container.style[attr]=`${rect[attr]}${typeof rect[attr]==='number'?'px':''}`)
                }
            }, ...args)
        ...
        if(this.contentRef.current) {
            ...
            fullScreenButton.onclick = () => toggleFullScreen(this.contentRef.current!, container)
        }
    }

    private closeWindow(windowRef: {command: Command, container: HTMLDivElement}) {
        if(!this.windows[windowRef.command.name]) return;

        this.windows[windowRef.command.name].find(x => x === windowRef)?.container.remove()
        this.windows[windowRef.command.name] = this.windows[windowRef.command.name].filter(x => x != windowRef)
    }

    private moveOnTop(windowRef: {command: Command, container: HTMLDivElement}) {
        Object.values(this.windows).forEach(wins => wins.forEach(win => win.container.classList.remove('top')))
...
const createWindoeHeader = (command: Command) => {
    ...
    const appendActionButton = (props: {icon: string, title: string, onClick: () => void}) => {
...
const appendWindow = (contentArea: HTMLDivElement, windowElement: HTMLElement) => {
    ...
    contentArea.querySelector(`:scope > .${WINDOWS_CONTAINER_CLASS}`)!.appendChild(windowElement)
}
