        ...
        <div style={{ display: 'flex', height: '100%', flexDirection: 'column' }} className="file-explorer">
            ...
            <div className="file-explorer-body">
                ...
                <section className="dir-list">
                    <ListDirComponent key={dirRef.current} dir={dirRef.current} openFile={openFile} showFileActions={showFileActionsHandler} />
...
const ListDirComponent = ({ dir, openFile, showFileActions }: { dir?: string, openFile: (file: FileType) => void, showFileActions: (file: FileType, event: React.MouseEvent<HTMLDivElement, MouseEvent>) => void }) => {
    dir ??= DESKTOP_PATH;
        ...
        <main className={`${DESKTOP_CONTAINER_CLASS}-files`} ref={containerRef}>
            {
                files.map(file => (
                    ...
                    >
                        <div className={`file`} data-ext={file.meta.ext} style={{ backgroundImage: `url('${file.type === 'file' && imageExtensions.has(file.meta.ext as string) && thumbnails[file.path] ? thumbnails[file.path] : extIconMap[file.meta.ext as string] ?? extIconMap['']}')`, backgroundRepeat: 'no-repeat', backgroundSize: 'cover', backgroundPosition: 'center center' }}></div>
